...
class TvRepositoryImpl implements TvRepository {
  ...
  Future<Either<Failure, List<Tv>>> getNowPlayingTvSeries() async {
    if (await networkInfo.isConnected) {
      try {
        ...
        localDataSource
            .cacheNowPlayingTvs(result.map((tv) => TvTable.fromDTO(tv)).toList());
  ...
  Future<Either<Failure, String>> saveWatchlist(TvDetail tv) async {
    try {
      final result = await localDataSource.insertTvWatchlist(TvTable.fromEntity(tv));
      return Right(result);
  ...
  Future<Either<Failure, String>> removeWatchlist(TvDetail tv) async {
    try {
      final result = await localDataSource.removeTvWatchlist(TvTable.fromEntity(tv));
      return Right(result);
    } on DatabaseException catch (e) {
      return Left(DatabaseFailure(e.message));
    }
